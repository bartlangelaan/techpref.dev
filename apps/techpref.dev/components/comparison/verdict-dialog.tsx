...
import { ChevronDown, ChevronRight, ExternalLink, FileCode, Star } from "lucide-react";
...
function SamplesList({ samples }: { samples: ViolationSample[] }) {
  if (samples.length === 0) {
    return <span className="text-muted-foreground text-xs">No samples available</span>;
  }
  ...
  return (
    <ul className="space-y-1 text-xs">
      ...
      {samples.length > 5 && (
        <li className="text-muted-foreground">...and {samples.length - 5} more</li>
      )}
    </ul>
  );
}
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-h-[85vh] max-w-2xl overflow-hidden flex flex-col">
        ...
        <div className="text-muted-foreground text-xs pt-3 border-t border-border">
